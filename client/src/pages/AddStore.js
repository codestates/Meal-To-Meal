...
function AddStore({ navigate, setAlertMessage, openWarningAlertHandler }) {
  ...
  const addMenuHandler = key => e => {
    // 메뉴 추가 핸들러
    // addMenuHandler([...menuInfo, { menu_price: e.target.value }]);
    // setMenuInfo([...menuInfo, { menu_name: item.menu_name, menu_price: item.menu_price }]);
    // setMenuInfo([...menuInfo, { [key]: e.target.value }]);
    const oneMenu = {
      menu_name: menuName,
      menu_price: menuPrice,
    };
    const newMenu = [oneMenu, ...menuInfo];
    setMenuInfo(newMenu);
  ...
  const newStoreRegisterHandler = () => {
    ...
    const { menu_name, menu_price } = menuInfo;
    axios
      .post(
      ...
      <div className="AddStore-container">
        <div className="AddStore-store-title-container">
          ...
          <input
            ...
            onChange={handleInputValue('store_name')}
          />
          <div className="AddStore-store-text">카테고리</div>
          <select className="AddStore-store-info input" onChange={handleInputValue('store_category')}>
          ...
          <textarea
            ...
            onChange={handleInputValue('store_description')}
          />
        </div>
        <div className="AddStore-store-title-container">
          ...
          <input
            ...
            onChange={handleInputValue('business_hour')}
          />
