...
module.exports = async (req, res) =>{
    try {
        const { imp_uid, merchant_uid,menu_id,store_id } = req.body;  // 클라이언트에서 구매버튼을 눌렀을때 유저 아이디와 스토어 아이디를 보내줘야함.
        ...
        const getToken = await axios({
          ...
          const userInfo = await user.findOne({where:{user_email : paymentData.buyer_email}}); // 클라이언트에서 요청을 보낼때 이것도 포함해서 보내줘야함.
          const menuInfo = await menu.findOne({where:{}}) //
          const storeInfo = await store.findOne({where:{}})//
        ...
        await cart.create({
          ...
          user_id : paymentData.buyer_email,
          store_id : store_id ,
          menu_id : menu_id ,
        })
